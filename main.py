...
def create_window():
    ...
    for x, y in resolutions:
        button = tk.Button(window, text=f"{x}x{y}", command=lambda x=x, y=y: on_resolution_button_click(x, y))
        button.pack(pady=5)

    extended_mode_button = tk.Button(window, text="Extended Mode", command=on_extended_mode_button_click)
    extended_mode_button.pack(pady=5)

    internal_mode_button = tk.Button(window, text="Internal Mode", command=on_internal_mode_button_click)
    internal_mode_button.pack(pady=5)

    refresh_rate_60_button = tk.Button(window, text="Set to 60Hz", command=lambda: on_refresh_rate_button_click(60))
    refresh_rate_60_button.pack(pady=5)

    refresh_rate_120_button = tk.Button(window, text="Set to 120Hz", command=lambda: on_refresh_rate_button_click(120))
    refresh_rate_120_button.pack(pady=5)

    refresh_rate_144_button = tk.Button(window, text="Set to 144Hz", command=lambda: on_refresh_rate_button_click(144))
